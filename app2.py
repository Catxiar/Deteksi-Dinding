...
# Fungsi utama aplikasi yang dipanggil setelah login berhasil
def main_app():
    ...
    st.sidebar.markdown(
        """
        Model ini dikembangkan menggunakan **Ultralytics YOLOv8** OBB.
    ...
    def process_image(file, model, conf_threshold, iou_threshold, imgsz_choice, selected_class_indices):
        ...
        # --- LOGIKA KLASIFIKASI SEVERITY BARU ---
        if severity > 90:
            severity_cat = 'Parah (Severe)'
        elif severity > 60:
            severity_cat = 'Besar (Major)'
        elif severity > 30:
            severity_cat = 'Sedang (Moderate)'
        elif severity > 0:
            severity_cat = 'Kecil (Minor)'
        else:
            severity_cat = 'Aman/Tidak Terdeteksi'
        # --- END LOGIKA KLASIFIKASI SEVERITY BARU ---
        ...
        for cls_name, count in detection_summary.items():
            ...
            # PENYEBAB & JENIS KERUSAKAN
            if cls_name == 'crack':
                note = 'Retak dapat mengindikasikan isu struktural tergantung kedalaman dan panjang. Penyebab umum: pergerakan tanah, beban berlebih, atau perubahan suhu ekstrem.'
            elif cls_name == 'mould':
                note = 'Mould berhubungan dengan masalah kelembapan dan bisa berdampak kesehatan. Penyebab umum: kebocoran pipa, kondensasi, atau ventilasi yang buruk.'
            else:
                note = ''
            ...
            type_severity_notes[cls_name] = {
                ...
                'note': note,
                'suggestion': repair_suggestions.get(cls_name, '') # PENANGANAN
            }
            ...
            for idx, r in enumerate(results_all):
                ...
                with st.expander(f"{rep['file_name']} — Severity: {rep['severity_score']:.1f} ({rep['severity_category']})", expanded=False):
                    ...
                    for k,v in rep['type_severity_notes'].items():
                        st.markdown(f"**Jenis Kerusakan: {k.upper()}**")
                        st.write(f"&nbsp;&nbsp;&nbsp;&nbsp;*Penyebab:* {v['note']}")
                        st.warning(f"&nbsp;&nbsp;&nbsp;&nbsp;*Saran Penanganan:* {v['suggestion']}")
